...
def plot_variables(df, last_snapshots={}):
    ...
    def add_checkbox_to_group(grp_name, col_name):
        ...
        if grp_name not in groups or not isinstance(groups[grp_name], dict):
            ...
            gf = tk.LabelFrame(scrollable_frame, text="", bd=1, relief="solid", bg="#Ecf0f1", padx=0, pady=0)
            ...
            btn_frame.pack(side="top", fill="x", padx=0, pady=0)
            
            
            symbol_var = tk.StringVar(value="▶") 

            def toggle(frame_to_toggle, symbol_var):
                if frame_to_toggle.winfo_ismapped():
                    ...
                    frame_to_toggle.pack(fill="x", padx=5, pady=(2, 5))
                    symbol_var.set("▼")

            toggle_cmd = lambda vf=vf, s_var=symbol_var: toggle(vf, s_var)
            
            
            symbol_lbl = tk.Label(btn_frame, textvariable=symbol_var, anchor="w", bg="#Ecf0f1", font=("Arial", 9, "bold"))
            symbol_lbl.pack(side="left", padx=(5, 2), pady=3)
    ...
    tk.Label(math_frame, text="2. Add operators here:").pack(padx=5)
    ...
    for r, row_ops in enumerate(ops):
        for c, op in enumerate(row_ops):
            btn = tk.Button(btn_grid, text=op, width=8, bg="#Ecf0f1",
                            command=lambda op=op: insert_text(op))
            btn.grid(row=r, column=c, padx=1, pady=1, sticky="ew")

    for c in range(max(len(row) for row in ops)):
         btn_grid.grid_columnconfigure(c, weight=1)

    tk.Button(math_frame, text="COMPUTE & PLOT", command=add_expression, bg="#bdc3c7", font=("Arial", 9, "bold")).pack(fill="x", padx=5, pady=10)
